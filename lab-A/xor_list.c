...
void add_to_list(const char *str) {
    ...
    if (head == NULL) {
        new_node->xor_address = xor_ptrs(NULL, NULL);
        head = new_node;
        tail = new_node;
    } else {
        new_node->xor_address = xor_ptrs(tail, NULL);
        tail->xor_address =
            xor_ptrs((Node *)xor_ptrs(tail->xor_address, NULL), new_node);
        tail = new_node;
    }
}

// Операция: Найти элемент по ключу
Node *find_by_key(const char *key) {
    ...
    while (curr != NULL) {
        ...
        next = (Node *)xor_ptrs(prev, (Node *)curr->xor_address);
        prev = curr;
        curr = next;
    }
    return NULL;
...
// Операция: Итерирование по списку и печать элементов
void iterate_list() {
    Node *curr = head;
    Node *prev = NULL;
    Node *next;
...
// Операция: Удалить элемент из списка по ключу
...
// Операция: Удалить узел по указателю
void remove_by_pointer(Node *node_to_delete) {
    ...
    while (curr != NULL && curr != node_to_delete) {
        next_node = (Node *)xor_ptrs(prev, (Node *)curr->xor_address);
        prev = curr;
        curr = next_node;
    }
    ...
    if (prev == NULL) {
        ...
        prev->xor_address =
            xor_ptrs((Node *)xor_ptrs(prev->xor_address, curr), next_node);
    }

    if (next_node == NULL) {
        tail = prev;
    } else {
        next_node->xor_address =
            xor_ptrs(prev, (Node *)xor_ptrs(next_node->xor_address, curr));
    }
